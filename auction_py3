...
def auction():
    print_details()
    buyer_no = str(input("Enter your buyer id: "))
    item_no = str(input("Which item do you want to place bid for: "))
    bid = int(input("Enter bid value(higher than current bid)"))
...
print ("************Auction***********")
print ("Define items")
no_items = int(input("Enter no. of items to define: "))
...
print
no_buyers = int(input("Enter number of buyers: "))
...
while auct:
    ...
    print( buyers_d)
    print()
    c = str(input("Continue Auction (y/n): "))
    if c == "n":
        auct = False

for it, it_dtls in items_d.items():
    if int(it_dtls[3]) >= int(it_dtls[1]): #if final bid is higher than reserved price for that item
